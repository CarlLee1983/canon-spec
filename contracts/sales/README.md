...
All twelve Clarification Requests were resolved by human domain decisions for this revision, so no recorded question remains open. An authorized human domain reviewer marked these contracts `accepted`. Compatibility is now judged against an accepted observer, so any later change that would make a conforming implementation non-conforming requires a major version.
